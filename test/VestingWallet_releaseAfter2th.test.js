...
contract("VestingWallet - release after 2th", ([owner, alice, bob]) => {
  beforeEach(async () => {
  ...
  context("Basics", () => {
    ...
    it("should be able to withdraw correctly after 2th and at 3th", async () => {
      (await this.band.balanceOf(alice)).toString().should.eq("0");
      (await this.band.balanceOf(this.vw.address))
        .toString()
        .should.eq("1000000");

      await time.increaseTo(1592380800 + 1); // after 2th
      ...
      await time.increaseTo(1600329600); // at 3th
      await this.vw.release({ from: alice });
