...
contract("VestingWallet - under tokens", ([owner, alice, bob]) => {
  beforeEach(async () => {
    ...
    await this.band.transfer(this.vw.address, 700000, { from: owner });
  });
  ...
  context("Basics", () => {
    ...
    it("should be able to withdraw correctly after 1th and at 2th, 3th", async () => {
      ...
      (await this.band.balanceOf(alice)).toString().should.eq("200000");
      (await this.band.balanceOf(this.vw.address))
        .toString()
        .should.eq("500000");
      ...
      await this.vw.release({ from: alice });
      (await this.band.balanceOf(alice)).toString().should.eq("450000");
      (await this.band.balanceOf(this.vw.address))
        .toString()
        .should.eq("250000");
      ...
      await this.vw.release({ from: alice });
      (await this.band.balanceOf(alice)).toString().should.eq("700000");
